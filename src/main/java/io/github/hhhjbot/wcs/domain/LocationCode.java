 ...
 * 화물이 놓이는 자리.
 *
 * <p>지금까지 출발지·목적지를 문자열로만 다뤘다. 문자열은 형식이 맞는지 아무도 검사하지 않고,
 * "A-1-3-2"와 "A-01-03-02"가 같은 자리인지도 알 수 없다. 자리를 값 객체로 만들어
 * 형식 검사와 해석을 한곳에 모은다.
 *
 * <h3>두 가지 종류</h3>
 * <pre>
 ...
 * </pre>
 *
 * 랙은 좌표를 갖고 스테이션은 이름만 갖는다. 그래도 <b>타입은 하나로 둔다.</b>
 ...
 *
 * <h3>담당 크레인을 여기서 도출하는 이유</h3>
 * 랙 주소에는 이미 존과 통로가 들어 있고, 통로마다 크레인이 한 대 선다.
 * 그래서 {@code A-01-03-02}를 꺼내려면 {@code SC-A01}이 움직인다는 사실이
 * 주소만 보고도 정해진다. 이 매핑을 별도 표로 관리하면 주소와 표가 어긋날 수 있다.
 * (ADR-0010)
 *
 * <h3>형식이 틀리면 예외</h3>
